...
class App {
  public async post_content(
    ...
    return new Promise(
      (resolve: (uuid: string) => void, reject: (err: Error) => void) => {
  ...
  public async import_script() {
    const data = fs.readFileSync(path.join(__dirname, "naf_v2.json"), "utf8");
    ...
    for (const nafArray of jsonContent) {
      ...
      for (const specialityItem of nafArray.SPECIALITY) {
        let speciality: { label: string; nafId: string } = {
          label: specialityItem,
          nafId: naf_id,
        };
        console.log(speciality);
        let specialty_id = await this.post_content(speciality, "/specialty");
